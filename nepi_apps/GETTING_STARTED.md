...
NodeClassIF skeleton, RUI injection), read `APP_STRUCTURE.md`.
...
1. **Copy the `nepi_app_template/` folder** into your own repo/location,
   **along with `deploy_nepi_apps.sh`** sitting next to it.
2. **Rename the folder** to `nepi_app_<your_name>` and rename every
   `template` / `Template` token across all files. The rename must stay
   consistent everywhere — `pkg_name` is used as the catkin package name, the
   rosrun package, AND the msg import module. The full checklist:
...
Also read the **Gotchas** section at the end of `APP_STRUCTURE.md` before your
first deploy.
