...
DRIVERS_FOLDER=$(cd -P "$(dirname -- "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)
DRIVERS_NAME=$(basename "$DRIVERS_FOLDER")

###############################################
# Deploy Nepi Drivers
###############################################
SOURCE_PATH=$DRIVERS_FOLDER
DEST_PATH="/mnt/nepi_storage/nepi_src/nepi_engine_ws/src/nepi_drivers/${DRIVERS_NAME}"
...
echo ""
echo "Syncing Drivers ${DRIVERS_NAME} from ${SOURCE_PATH} to NEPI Build Repo at:" 
...
echo ""
echo "Syncing Drivers ${DRIVERS_NAME} from ${SOURCE_PATH} to NEPI Live Folders at:" 
echo "Destination Path ${DEST_PATH}"
echo ""
